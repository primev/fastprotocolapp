...
type TaskConfig = {
  ...
  requiresSBT?: boolean;
  requiresConnectWalletCompleted: boolean;
};
...
const TASK_CONFIG: Record<TaskName, TaskConfig> = {
  'Connect Wallet': {
    ...
    requiresConnectWalletCompleted: false,
  },
  'Fast RPC Setup': {
    fieldName: 'setup_rpc_completed',
    requiresWallet: true,
    requiresConnection: false,
    requiresConnectWalletCompleted: true,
  },
  'Mint Genesis SBT': {
...
const DashboardContent = () => {
  ...
  const [processingTasks, setProcessingTasks] = useState<Set<TaskName>>(new Set());
  ...
  useEffect(() => {
    ...
    if (tokenIdFromQuery && tokenId !== undefined && tokenId !== BigInt(0)) {
      // Show feedback modal when SBT is minted
  ...
  const handleEmailSubmit = () => {
    if (validateEmail(emailInput)) {
      ...
      handleTaskComplete('Enter Email' as TaskName);
  ...
  const updateUserOnboarding = async (walletAddress: string, updates: Record<string, boolean>) => {
  ...
  useEffect(() => {
    if (isConnected && address) {
      ...
      setUserOnboarding(null);
      setHasInitializedOnboarding(true); // Mark as initialized even when disconnected
    }
  }, [isConnected, address]);
  ...
  const validateTaskCompletion = (taskName: TaskName): { valid: boolean; error?: string } => {
    ...
    // Check if Connect Wallet must be completed first
    if (config.requiresConnectWalletCompleted && !userOnboarding?.connect_wallet_completed) {
      return { valid: false, error: 'Please complete "Connect Wallet" task first' };
    }

    // Check wallet connection requirements
    if (config.requiresWallet && !address) {
      return { valid: false, error: 'Please connect your wallet first' };
    }

    // Check active connection requirement
  ...
  const handleTaskComplete = async (taskName: TaskName) => {
    ...
    // Optimistically hide the task card
    setProcessingTasks(prev => new Set(prev).add(taskName));

    try {
      const success = await updateUserOnboarding(address, {
  ...
  const getTaskCompleted = (taskName: TaskName): boolean => {
    // Special handling for Mint Genesis SBT - check hasGenesisSBT variable
    if (taskName === 'Mint Genesis SBT') {
      return hasGenesisSBT;
    }
  ...
  // // Update accordion state when database record is present and all tasks are completed
  // useEffect(() => {
  //   if (hasInitializedOnboarding) {
  //     const allCompleted = oneTimeTasks.every(task => task.completed);
  //     if (allCompleted) {
  //       setOneTimeTasksAccordionValue(undefined); // Collapse when all tasks are completed
  //     } else {
  //       setOneTimeTasksAccordionValue("one-time-tasks"); // Keep open if there are incomplete tasks
  //     }
  //   }
  // }, [hasInitializedOnboarding, userOnboarding, hasGenesisSBT]);

  // Debug: Log oneTimeTasks on page load (remove in production)
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('One-Time Tasks on page load:', oneTimeTasks);
    }
  }, []);

  // Debug: Log oneTimeTasks whenever userOnboarding or hasGenesisSBT changes (remove in production)
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('One-Time Tasks after user record change:', oneTimeTasks);
      console.log('User onboarding data:', userOnboarding);
    }
  }, [userOnboarding, hasGenesisSBT, oneTimeTasks]);
    ...
    <div className="min-h-screen bg-background relative overflow-hidden">
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <Accordion 
                    type="single" 
                    collapsible 
                    value={oneTimeTasksAccordionValue}
                    onValueChange={setOneTimeTasksAccordionValue}
                    ...
                    <AccordionItem value="one-time-tasks">
                      ...
                      <AccordionContent className="px-6 pb-6">
                        <div className="space-y-3">
                          {oneTimeTasks.filter(task => !processingTasks.has(task.name as TaskName)).map((task) => (
                              <div
                                key={task.name}
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        if (task.action === 'email') {
                                          // Check if Connect Wallet is completed first
                                          if (!userOnboarding?.connect_wallet_completed) {
                                            toast.error('Please complete "Connect Wallet" task first');
                                            return;
                                          }
                                          ...
                                          setTimeout(
                                            () => handleTaskComplete(task.name as TaskName),
                                            1000
                                          );
                                        } else {
                                          handleTaskComplete(task.name as TaskName);
      ...
      <MetaMaskToggleModal
        ...
        onComplete={() => {
          ...
          // RPC setup completion will be tracked when test is completed
        }}
      />
