      ...
      <Dialog open={isHelpDialogOpen} onOpenChange={setIsHelpDialogOpen}>
        <DialogContent className="rounded-[40px] p-12 border-0 shadow-3xl">
          ...
          <DialogDescription className="text-lg text-muted-foreground leading-relaxed mt-4 font-medium">
            Fast Protocol is a MEV-sharing protocol + RPC stack built on top of mev‑commit. It is
            mathematically constrained to pay back the highest level of MEV rebates to users, which
            can exceed the amount their transactions generate (&gt;100%).
          </DialogDescription>
        </DialogContent>
      </Dialog>
...
const formatCurrency = (v: number | null) =>
  v
    ...
    : "—"
const formatEth = (v: number | null) =>
  v ? `${v.toLocaleString(undefined, { maximumFractionDigits: 1 })} ETH` : "—"
...
function MetricTile({ label, value, accent, isLoading }: any) {
  return (
    <div className="bg-[#0D0D0E] p-12 flex flex-col justify-between hover:bg-white/[0.02] transition-colors group">
      <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 mb-10 group-hover:text-white/40">
        {label}
      </span>
      <div
        className={cn(
