...
export function useAffiliateCode(): UseAffiliateCodeReturn {
  ...
  const loadAffiliateCode = useCallback(async () => {
    ...
    try {
      ...
      const code = typeof result === 'string'
        ? result
        : (result as unknown as { code?: string })?.code || null;
  ...
  const generateReferralLink = useCallback(async (codeOverride?: string | null) => {
    if (!address) {
      setReferralLink('');
      return;
    }
  ...
  const createAffiliateCode = useCallback(async (code: string): Promise<boolean> => {
    if (!address) {
  ...
  const updateAffiliateCode = useCallback(async (code: string): Promise<boolean> => {
    if (!address) {
      toast.error('Please connect your wallet');
      return false;
    }
