...
export const CommunityStepsModal = forwardRef<CommunityStepsModalRef, CommunityStepsModalProps>(
  ({ open, onOpenChange, onAllStepsCompleted, onEmailDialogOpen }, ref) => {
    ...
    useEffect(() => {
      if (
        allStepsCompleted &&
        completedSteps.size === communitySteps.length &&
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="w-full h-full sm:h-auto sm:max-w-xl border-primary/50 max-h-[100vh] sm:max-h-[90vh] flex flex-col m-0 sm:m-4 rounded-none sm:rounded-lg inset-0 sm:inset-auto sm:left-[50%] sm:top-[50%] translate-x-0 translate-y-0 sm:translate-x-[-50%] sm:translate-y-[-50%] overflow-y-auto p-4 sm:p-6">
          <DialogHeader>
