...
description: "Fast Protocol returns at least 90% of the mev your swap generates. Learn how the order flow auction redistributes value back to users."
slug: "mev-rewards"
category: "mev"
date: "2026-03-01"
...
### Before Fast Protocol

You swap → searchers extract mev from your transaction → builders capture a share → validators get the rest → you get nothing beyond the raw swap output.

### With Fast Protocol

You swap → builders compete for your order flow → at least 90% of generated mev improves your execution → the remaining ~10% covers builder margins and protocol fees → you get a measurably better outcome.
...
### Key differences from DeFi yield
...
mev rewards are the per-swap redistribution — you earn them automatically on trades that generate mev. [Miles](/learn/miles) represent the cumulative loyalty layer built on top of this same activity.

Swaps that generate mev rewards also earn Miles. The mev reward improves your immediate execution. Miles accumulate as a record of your total mev contribution — compounding into value over time. Note that not every swap generates mev; it depends on pair volatility, liquidity depth, swap size, and market conditions.
